package tech.optimind.flutter.network_usage
...
import tech.optimind.flutter.network_usage.Utils.drawableToBase64
import tech.optimind.flutter.network_usage.Utils.getValueOrDefault

/** NetworkUsagePlugin */
class NetworkUsagePlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    ...
    override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
        channel = MethodChannel(flutterPluginBinding.binaryMessenger, "network_usage")
    ...
    private fun handleGetDataUsage(result: MethodChannel.Result, call: MethodCall) {
        if (isAccessGranted()) {
            try {
                getDataFromPhone(
                    result,
                    getValueOrDefault(call.argument<Boolean>("withAppIcon"), false),
                    getValueOrDefault(call.argument<Boolean>("isWifi"), false)
                )
            } catch (e: Exception) {
                result.error("NETWORK_USAGE_ERROR", e.message, e.toString())
            }
        } else {
            result.error("NETWORK_USAGE_ERROR", ".init() was not called", "Please call init first")
        }
    }
    ...
    private fun getDataFromPhone(
        ...
        @SuppressLint("HardwareIds") val subscriberId =
    ...
    private fun getDataFromOldPhone(result: MethodChannel.Result, call: MethodCall) {
        ...
        try {
            for (_package in packages) {
                ...
                data["package_name"] = _package.packageName
                if (withIcons) {
                    val icon = pm.getApplicationIcon(
                        pm.getApplicationInfo(
                            _package.processName,
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        ...
        Handler(Looper.getMainLooper()).post {
            ...
            when (call.method) {
                ...
                "getNetworkUsageOld" -> if (permissionGranted) {
